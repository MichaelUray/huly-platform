...
<!--
  ...
  true, inside an out-of-flow overlay so it never displaces the still-live
  viewlet (see the .search-empty-state-overlay comment in IssuesView).
  Because it floats above the viewlet it carries its own surface
  (background/border/shadow) instead of inheriting the page background.
  Buttons: clear-all-filters (only when filters are active), and
