...
-->
<!--
  Phase 2.5 — Workspace Access Center "Guest Settings" tab body.

  This is the verbatim editor that used to live in
  `GuestPermissionsSettings.svelte` (now a thin compat-shim), extracted
  so it can be mounted from two places:

    1. The legacy `/setting/guestPermissions` route — the shim mounts
       AccessCenter on the `guest-settings` tab, which renders this
       component via `<Component is={setting.component.GuestPermissionsEditor}/>`.
    2. The new WAC 5th tab `Guest Settings` (Owner-only) inside the
       Access Center surface itself.

  Chrome (Header + Breadcrumb) is intentionally NOT rendered here — the
  AccessCenter shell already provides it. The component renders its own
  two-column NavItem layout (Guest / Anonymous Guest) inside the
  caller's content area.

  Behaviour-identical to the pre-Phase-2 editor. The underlying Doc
  schema (`core.class.ModulePermissionGroup`, account-client
  `allowReadOnlyGuest` + `allowGuestSignUp`, `communication.class.GuestCommunicationSettings`)
  is unchanged.

  Operator-write gating: `canEdit` prop, default `true` for backward
  compatibility with the legacy mount. WAC always passes the
  role-resolved value (Owner = true, everyone else = false). When
  `canEdit` is false the toggles still render but are disabled — same
  visual pattern as `anonymousModulePermissionsReadOnly` already in
  this file.
-->
<script lang="ts">
  ...
  /**
   * Operator-write gate. When `false` every toggle and embedded editor
   * renders disabled. Defaults to `true` so the legacy mount (no prop
   * passed) behaves exactly as before. WAC always passes the
   * role-resolved value.
   */
  ...
  /** Combined disable predicate — caller may force read-only via canEdit=false. */
  $: globalReadOnly = !canEdit
...
<div class="guestPermissionsEditor" data-test="guest-settings-editor">
  <div class="guestPermissionsEditor-columns">
    <div class="guestPermissionsEditor-column navigation py-2">
      <Scroller shrink>
        <NavItem
    ...
    <div class="guestPermissionsEditor-column content">
      {#if loading}
      ...
      {:else}
        <Scroller align={'center'} padding={'var(--spacing-3)'} bottomPadding={'var(--spacing-3)'}>
          <div class="guestPermissionsRoot flex-col">
            ...
            <section class="section">
              ...
              <div class="cardStack" class:cardStack-readonly={anonymousModulePermissionsReadOnly || globalReadOnly}>
...
<style lang="scss">
  ...
  .guestPermissionsEditor-columns {
    display: flex;
    flex-direction: row;
    width: 100%;
    min-height: 28rem;
  ...
  .guestPermissionsEditor-column {
    display: flex;
    flex-direction: column;
    min-height: 0;
  }

  .guestPermissionsEditor-column.navigation {
    flex: 0 0 14rem;
    border-right: 1px solid var(--theme-divider-color);
  }

  .guestPermissionsEditor-column.content {
    flex: 1 1 auto;
    min-width: 0;
  }
